...
from cinema.data.dicom import check_lax_sax_images, concatenate_sax_images, load_dicom_folder
...
def split_dicom_files_and_convert_to_nifti(
    ...
    for series_name, series_df in manifest_df.groupby("series discription"):
        ...
        image = load_dicom_folder(series_dir)
        series_name_to_image[series_name] = image
        sitk.WriteImage(image, nifti_dir / f"{eid}_{instance_id}_{series_name}.nii.gz")
...
def transform_to_nifti(
    ...
    sax_series_name_to_image, sax_manifest_df = split_dicom_files_and_convert_to_nifti(
    ...
    nums = get_sax_series(sax_manifest_df, folder_id)
    sax_images = [sax_series_name_to_image[f"CINE_segmented_SAX_b{i}"] for i in nums]

    sax_slice_spacing = check_lax_sax_images(
        lax_2c_image=lax_2c_image,
        lax_3c_image=lax_3c_image,
        lax_4c_image=lax_4c_image,
        sax_images=sax_images,
        image_id=folder_id,
    )
    # combine SAX images, GetSize() = (x, y, z, t)
    sax_image = concatenate_sax_images(sax_images, sax_slice_spacing)
...
def crop_nifti(
    ...
    # resample images
    lax_2c_image = resample_spacing_3d(
        image=lax_2c_image,
        is_label=False,
        target_spacing=(*spacing[:2], lax_2c_image.GetSpacing()[-1]),
    )
    lax_3c_image = resample_spacing_3d(
        image=lax_3c_image,
        is_label=False,
        target_spacing=(*spacing[:2], lax_3c_image.GetSpacing()[-1]),
    )
    lax_4c_image = resample_spacing_3d(
        image=lax_4c_image,
        is_label=False,
        target_spacing=(*spacing[:2], lax_4c_image.GetSpacing()[-1]),
    )
    ...
    lax_3c_rot = np.array(lax_3c_image.GetDirection()).reshape((3, 3))
    lax_3c_center = point_to_plane_projection(
        point=sax_center,
        plane_origin=lax_3c_image.GetOrigin(),
    ...
    lax_2c_image = clip_and_normalise_intensity_3d(lax_2c_image, intensity_range=None)
    lax_3c_image = clip_and_normalise_intensity_3d(lax_3c_image, intensity_range=None)
    lax_4c_image = clip_and_normalise_intensity_3d(lax_4c_image, intensity_range=None)
    ...
    sitk.WriteImage(
        ...
        fileName=out_dir / f"{folder_id}_lax_2c.nii.gz",
        useCompression=True,
    )
    sitk.WriteImage(
        image=cast_to_uint8(lax_3c_image),
        fileName=out_dir / f"{folder_id}_lax_3c.nii.gz",
        useCompression=True,
    )
    sitk.WriteImage(
        image=cast_to_uint8(lax_4c_image),
        fileName=out_dir / f"{folder_id}_lax_4c.nii.gz",
        useCompression=True,
    )
    sitk.WriteImage(
        image=cast_to_uint8(sax_image),
        fileName=out_dir / f"{folder_id}_sax.nii.gz",
        useCompression=True,
    )
