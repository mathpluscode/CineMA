...
from cinema.segmentation.mnms2.train import load_dataset as load_mnms2_dataset
from cinema.segmentation.rescan.ef_eval import ef_eval_rescan_dataset
from cinema.segmentation.rescan.eval import segmentation_eval_rescan_dataset
from cinema.segmentation.rescan.train import load_dataset as load_rescan_dataset
...
def test_seg_train_eval(mocker: MockFixture, model: str) -> None:
    ...
    config.data.dir = cache_dir / "rescan" / "processed"
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        run_train(
            config=config,
            load_dataset=load_rescan_dataset,
...
@pytest.mark.integration
@pytest.mark.parametrize("model", ["unet", "convunetr"])
@pytest.mark.parametrize("views", ["lax_4c", "sax"])
def test_lax_train_eval(mocker: MockFixture, model: str, views: str | list[str]) -> None:
    """Test training.
