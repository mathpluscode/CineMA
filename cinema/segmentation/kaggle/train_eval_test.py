...
from cinema.segmentation.acdc.train import load_dataset, segmentation_loss
from cinema.segmentation.kaggle.eval import segmentation_eval_kaggle_dataset
from cinema.segmentation.train import get_segmentation_model, segmentation_eval_dataloader
...
def get_test_config() -> DictConfig:
    """Get test config."""
    config_path = Path(__file__).parent.parent.resolve() / "acdc" / "config.yaml"
...
def test_seg_train_eval(mocker: MockFixture, model: str) -> None:
    ...
    config.data.dir = cache_dir / "acdc" / "processed"
    kaggle_data_dir = cache_dir / "kaggle" / "processed"
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        config.data.dir = kaggle_data_dir
