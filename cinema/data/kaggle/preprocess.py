...
def process_study(  # pylint:disable=too-many-statements
    ...
    lax_2c_image = load_dicom_folder(dir_2c)  # GetSize() = (x, y, t)
    lax_4c_image = load_dicom_folder(dir_4c)  # GetSize() = (x, y, t)

    # load all sax images
    sax_dirs = list(study_dir.glob("sax_*"))
    sax_dirs.sort(key=lambda x: int(x.name.split("sax_")[1]))
    sax_images = [load_dicom_folder(sax_dir) for sax_dir in sax_dirs]
    sax_images = filter_sax_images(sax_images, decimals)  # keep a subsequence if possible
    if len(sax_images) == 0:
        raise ValueError(f"No more SAX images after filtering for {pid}.")
